/*
  # Re-run Trust Score Backport After UUID Fix

  1. Problem
    - Previous backport attempt failed due to UUID casting issues in update_trust_score function
    - Historical data still needs trust scores backported
    - The function is now fixed to handle UUID casting properly

  2. Solution
    - Re-run the exact same backport logic now that the function works
    - Use the same point values and metadata structure
...
BEGIN
    RAISE LOG 'Re-running backport for community creation trust scores';
    ...
    RAISE LOG 'Completed re-run backport for community creation trust scores';
END
$$;
...
BEGIN
    RAISE LOG 'Re-running backport for community joining trust scores';
    ...
    RAISE LOG 'Completed re-run backport for community joining trust scores';
END
$$;
...
DECLARE
    ...
    community_record RECORD;
BEGIN
    RAISE LOG 'Re-running backport for resource offer trust scores';
    ...
    RAISE LOG 'Completed re-run backport for resource offer trust scores';
END
$$;
...
BEGIN
    RAISE LOG 'Re-running backport for shoutout trust scores';
    ...
    RAISE LOG 'Completed re-run backport for shoutout trust scores';
END
$$;
