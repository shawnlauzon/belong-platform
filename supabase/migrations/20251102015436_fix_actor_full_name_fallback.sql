-- Fix actor_full_name to use fallback when full_name is NULL
-- For email signup users, compute full_name from first_name + last_name
...
SELECT
    ...
    n.conversation_id,
    n.actor_id,
    -- Merge actor info into metadata with full_name fallback
    CASE
        WHEN n.actor_id IS NOT NULL THEN
            COALESCE(n.metadata, '{}'::jsonb) || jsonb_build_object(
                'actor_display_name', COALESCE(up_actor.first_name, up_actor.full_name),
