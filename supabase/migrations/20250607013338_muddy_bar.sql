/*
  ...
  3. Data
    - Insert default hierarchical community structure
    - Worldwide > Countries > Cities > Neighborhoods
    - Uses proper v7 UUIDs for better performance
*/
...
-- Create policies (with IF NOT EXISTS equivalent using DO blocks)
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'communities' 
    AND policyname = 'Creators can update their communities'
  ) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'communities_parent_id_idx') THEN
    CREATE INDEX communities_parent_id_idx ON communities(parent_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'communities_level_idx') THEN
    CREATE INDEX communities_level_idx ON communities(level);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'communities_creator_id_idx') THEN
    CREATE INDEX communities_creator_id_idx ON communities(creator_id);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'communities_center_idx') THEN
    CREATE INDEX communities_center_idx ON communities USING gist(center);
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'communities_created_at_idx') THEN
