/*
  ...
  1. Profile Updates
    - Updates user_metadata for each user based on their email
    - Adds names, avatar URLs, and locations

  2. Resource Inserts
    - Inserts resources for each user
    - Uses actual user IDs from auth.users table
    - Includes all mock data (images, locations, descriptions)

  3. Data Integrity
    - Uses actual user IDs to avoid foreign key violations
    - Only processes users that exist in auth.users
*/
...
BEGIN
  ...
  -- Insert additional resource for Sarah (Sourdough Bread)
  INSERT INTO resources (
    creator_id,
    type,
    category,
