/*
  # Fix action_id UUID casting in update_trust_score function

  1. Problem
    - The update_trust_score function accepts p_action_id as TEXT
    - But trust_score_logs.action_id column is UUID type
    - Insertions fail with "column 'action_id' is of type uuid but expression is of type text"

  2. Solution
    - Cast p_action_id to UUID when inserting into trust_score_logs
    - This completes the UUID casting fix for the update_trust_score function

  3. Benefits
    - Allows the backport to complete successfully
    - Maintains proper type safety for UUID operations
    - Fixes the remaining UUID casting issue
*/

-- Fix the update_trust_score function to cast action_id to UUID
...
IS 'Centralized function for updating trust scores and logging all changes with proper error handling. Handles UUID casting for all UUID columns while accepting TEXT parameters for compatibility.';