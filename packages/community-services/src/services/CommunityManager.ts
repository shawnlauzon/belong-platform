...
export class CommunityManager {
  static initialize() {
    ...
    eventBus.on('community.create.requested', async (event) => {
      ...
      logger.debug(
        '🏘️ CommunityManager: Community creation requested:',
        communityData
      );

      try {
        const newCommunity =
          await this.createCommunityWithHierarchy(communityData);
  ...
  static async createCommunityWithHierarchy(communityData: {
    ...
    logger.debug(
      '🏘️ CommunityManager: Creating community with hierarchy:',
      communityData
    );
    ...
    if (communityData.state) {
      let existingStateId = communities.find(
        (c) =>
          c.level === 'state' &&
    ...
    logger.info(
      '✅ CommunityManager: Community created successfully with full hierarchy:',
      {
