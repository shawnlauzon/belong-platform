...
import type { Thanks, ThanksData, ThanksInfo, ThanksFilter } from '@belongnetwork/types';
import { 
  toDomainThanks, 
  toThanksInfo, 
  forDbInsert, 
...
export const createThanksService = (supabase: SupabaseClient<Database>) => ({
  ...
  async fetchThanksById(id: string): Promise<Thanks | null> {
    ...
    try {
      ...
      if (error) {
        ...
        logger.error('🙏 Thanks Service: Failed to fetch thanks', { id, error });
        throw error;
      }
  ...
  async createThanks(data: ThanksData): Promise<Thanks> {
    ...
    try {
      ...
      if (userError || !userData?.user?.id) {
        logger.error('🙏 Thanks Service: User must be authenticated to create thanks', {
          error: userError,
        });
  ...
  async updateThanks(id: string, data: Partial<ThanksData>): Promise<Thanks> {
    ...
    try {
      ...
      if (userError || !userData?.user?.id) {
        logger.error('🙏 Thanks Service: User must be authenticated to update thanks', {
          error: userError,
        });
        throw new Error(MESSAGE_AUTHENTICATION_REQUIRED);
      }
      ...
      if (error) {
        logger.error('🙏 Thanks Service: Failed to update thanks', { id, error });
        throw error;
      }
  ...
  async deleteThanks(id: string): Promise<void> {
    ...
    try {
      ...
      if (userError || !userData?.user?.id) {
        logger.error('🙏 Thanks Service: User must be authenticated to delete thanks', {
          error: userError,
        });
        throw new Error(MESSAGE_AUTHENTICATION_REQUIRED);
      }
      ...
      if (fetchError) {
        if (fetchError.code === 'PGRST116') {
          ...
          logger.debug('🙏 Thanks Service: Thanks not found for deletion', { id });
          return;
        }
      ...
      if (existingThanks.from_user_id !== userId) {
        logger.error('🙏 Thanks Service: User is not authorized to delete this thanks', {
          userId,
