...
describe('ThanksInfo Transformer', () => {
  it('should transform database thanks to ThanksInfo without snake_case properties', () => {
    ...
    // Act
    const result = toThanksInfo(dbThanks, 'user-123', 'user-456', 'resource-789', 'community-123');
    ...
    expect(result).toHaveProperty('impactDescription', 'This really helped me fix my bike');
  ...
  it('should handle optional fields correctly in ThanksInfo', () => {
    ...
    // Act
    const result = toThanksInfo(dbThanks, 'user-456', 'user-789', 'resource-123', 'community-456');
