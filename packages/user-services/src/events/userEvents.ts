...
} from '@belongnetwork/core/types/events';
...
export function initializeAuthEventHandlers() {
  ...
  eventBus.on('auth.signIn.requested', async (event: AuthSignInRequestedEvent) => {
    ...
    try {
      ...
      logger.error('❌ UserServices: Sign in handler exception', { error: errorMessage });
  ...
  eventBus.on('auth.signUp.requested', async (event: AuthSignUpRequestedEvent) => {
    ...
    try {
      ...
      logger.error('❌ UserServices: Sign up handler exception', { error: errorMessage });
  ...
  eventBus.on('auth.signOut.requested', async (event: AuthSignOutRequestedEvent) => {
    ...
    try {
      ...
      logger.error('❌ UserServices: Sign out handler exception', { error: errorMessage });
