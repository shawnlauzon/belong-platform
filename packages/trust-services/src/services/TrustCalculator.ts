...
export class TrustCalculator {
  static async calculateScore(memberId: string): Promise<number> {
    logger.debug('🛡️ TrustCalculator: Calculating score for member:', {
      memberId,
    });
    ...
    try {
      ...
      if (thanksError) {
        logger.warn(
          '🛡️ TrustCalculator: Error getting thanks count:',
          thanksError
        );
      }
      ...
      if (resourcesError) {
        logger.warn(
          '🛡️ TrustCalculator: Error getting resources count:',
          resourcesError
        );
      }
      ...
      const totalScore = Math.min(
        5.0 + tenureBonus + thanksBonus + sharingBonus,
        10.0
      );
  ...
  static getTrustTier(
    score: number
  ): 'New' | 'Building' | 'Established' | 'Exemplary' {
    logger.trace('🛡️ TrustCalculator: Getting trust tier for score:', {
      score,
    });
  ...
  static async getBreakdown(memberId: string) {
    logger.debug('🛡️ TrustCalculator: Getting breakdown for member:', {
      memberId,
    });
    ...
    try {
      ...
      if (thanksError) {
        logger.warn(
          '🛡️ TrustCalculator: Error getting thanks count for breakdown:',
          thanksError
        );
      }
      ...
      if (resourcesError) {
        logger.warn(
          '🛡️ TrustCalculator: Error getting resources count for breakdown:',
          resourcesError
        );
      }
      ...
      if (profileError) {
        logger.warn(
          '🛡️ TrustCalculator: Error getting profile for breakdown:',
          profileError
        );
      }
