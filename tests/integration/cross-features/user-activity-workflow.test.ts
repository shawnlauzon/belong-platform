...
describe('User Activity Workflow Integration', () => {
  ...
  test('should complete new member onboarding workflow', async () => {
    ...
    try {
      ...
      await waitFor(
        () => {
          const communityResources = createResourceResult.current.resources.data;
          const found = communityResources?.some(r => r.id === firstResource.id);
  ...
  test('should complete community event participation workflow', async () => {
    ...
    try {
      ...
      const { result: memberJoinResult } = await testUtils.renderHookWithWrapper(() => ({
      ...
      await waitFor(
        () => {
          ...
          const found = communityEvents?.some(e => e.id === communityEvent.id);
          expect(found).toBe(true);
        },
        { timeout: 10000 }
      );
      ...
      const { result: newMemberJoinResult } = await testUtils.renderHookWithWrapper(() => 
        useJoinEvent()
      );
  ...
  test('should complete resource sharing and appreciation workflow', async () => {
    ...
    try {
      ...
      const { result: shareResourceResult } = await testUtils.renderHookWithWrapper(() => 
        useCreateResource()
      );
      ...
      const sharedResource = await testUtils.performAsyncAction(
        () => shareResourceResult.current({
          ...
          description: 'Free tutoring sessions for community members - happy to help!',
          type: 'offer',
          category: 'skills',
          communityId: testCommunity.id,
          imageUrls: [],
      ...
      const { result: conversationResult } = await testUtils.renderHookWithWrapper(() => ({
      ...
      const initialMessage = await testUtils.performAsyncAction(
        () => conversationResult.current.sendMessage.mutateAsync({
          ...
          content: 'Hi! I saw your tutoring offer and would love to learn more. When are you available?',
        }),
      ...
      await testUtils.performAsyncAction(
        () => responseResult.current.mutateAsync({
          conversationId: resourceConversation.id,
          content: 'Absolutely! I\'m available weekday evenings. What subject would you like help with?',
        }),
      ...
      const appreciationShoutout = await testUtils.performAsyncAction(
        () => shoutoutResult.current.mutateAsync({
          message: TestDataFactory.generateTestName('APPRECIATION') + ' - Thanks for offering to help! This community is amazing.',
      ...
      const { result: verifyShoutoutResult } = await testUtils.renderHookWithWrapper(() => 
      ...
      await waitFor(
        () => {
          ...
          const found = resourceShoutouts?.some(s => s.id === appreciationShoutout.id);
          expect(found).toBe(true);
        },
        { timeout: 10000 }
      );
  ...
  test('should complete community leader appreciation workflow', async () => {
    ...
    try {
      ...
      const { result: specialEventResult } = await testUtils.renderHookWithWrapper(() => 
        useCreateEvent()
      );
      ...
      const specialEvent = await testUtils.performAsyncAction(
        () => specialEventResult.current.mutateAsync({
          ...
          description: 'Community appreciation event - celebrating our amazing members!',
      ...
      const { result: activeMemberJoinResult } = await testUtils.renderHookWithWrapper(() => 
        useJoinEvent()
      );
      ...
      const { result: activeShoutoutResult } = await testUtils.renderHookWithWrapper(() => 
        useCreateShoutout()
      );

      await testUtils.performAsyncAction(
        () => activeShoutoutResult.current.mutateAsync({
          message: TestDataFactory.generateTestName('LEADER_THANKS') + ' - Thank you for creating such a welcoming community!',
      ...
      const { result: newShoutoutResult } = await testUtils.renderHookWithWrapper(() => 
        useCreateShoutout()
      );

      await testUtils.performAsyncAction(
        () => newShoutoutResult.current.mutateAsync({
          message: TestDataFactory.generateTestName('LEADER_APPRECIATION') + ' - Your leadership makes this community special!',
          fromUserId: newMember.userId,
          toUserId: communityOwner.userId,
          isPublic: true,
        }),
      ...
      const { result: ownerShoutoutsResult } = await testUtils.renderHookWithWrapper(() => 
  ...
  test('should verify cross-feature data consistency throughout workflows', async () => {
    ...
    try {
      ...
      await waitFor(
        () => {
          expect(ownerViewResult.current.communityResources.isLoading).toBe(false);
          expect(ownerViewResult.current.communityEvents.isLoading).toBe(false);
          expect(ownerViewResult.current.communityShoutouts.isLoading).toBe(false);
      ...
      if (resources && resources.length > 0) {
        const allFromCommunity = resources.every(r => r.communityId === testCommunity.id);
        expect(allFromCommunity).toBe(true);
      }

      // Should have events from workflow tests
      if (events && events.length > 0) {
        const allFromCommunity = events.every(e => e.communityId === testCommunity.id);
        expect(allFromCommunity).toBe(true);
      }
      ...
      const { result: memberViewResult } = await testUtils.renderHookWithWrapper(() => ({
        communityResources: useResources({ communityId: testCommunity.id }),
        communityEvents: useEvents({ communityId: testCommunity.id }),
      ...
      await waitFor(
        () => {
          expect(memberViewResult.current.communityResources.isLoading).toBe(false);
          expect(memberViewResult.current.communityEvents.isLoading).toBe(false);
          expect(memberViewResult.current.publicShoutouts.isLoading).toBe(false);
        },
        { timeout: 15000 }
      );
    ...
    try {
      ...
      for (const activity of activities) {
        ...
        if (activity.action === 'create_resource') {
          const { result } = await testUtils.renderHookWithWrapper(() => useCreateResource());
          ...
          createdItems.push({ type: 'resource', item: resource, creator: activity.user });
        } else if (activity.action === 'create_event') {
          const { result } = await testUtils.renderHookWithWrapper(() => useCreateEvent());
          ...
          createdItems.push({ type: 'event', item: event, creator: activity.user });
        }
      }
      ...
      for (const created of createdItems) {
        for (const user of [communityOwner, activeMember, newMember]) {
          ...
          if (created.type === 'event') {
            // Users join events
            try {
              const { result } = await testUtils.renderHookWithWrapper(() => useJoinEvent());
            ...
            try {
              const { result } = await testUtils.renderHookWithWrapper(() => useCreateShoutout());
              
              await testUtils.performAsyncAction(
                () => result.current.mutateAsync({
                  message: TestDataFactory.generateTestName('ENGAGEMENT') + ' - This looks interesting!',
              ...
              console.log('Shoutout creation failed (acceptable):', error.message);
            }
          }
        }
      }
      ...
      const totalResources = finalViewResult.current.allResources.data?.length || 0;
      const totalEvents = finalViewResult.current.allEvents.data?.length || 0;
      const totalShoutouts = finalViewResult.current.allShoutouts.data?.length || 0;

      console.log(`Community activity summary: ${totalResources} resources, ${totalEvents} events, ${totalShoutouts} shoutouts`);

      // Should have meaningful activity from the simulation
      expect(totalResources + totalEvents + totalShoutouts).toBeGreaterThan(0);

      console.log('✅ Realistic user engagement patterns simulated successfully');
    } catch (error) {
