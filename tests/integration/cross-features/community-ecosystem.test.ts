...
describe.skip("Community Ecosystem Integration", () => {
    ...
    await waitFor(
      () => {
        ...
        const found = communityResources?.some(
          (resource) =>
            resource.id === communityResource.id &&
            resource.communityId === testCommunity.id,
        );
  ...
  test("should create shoutouts for community resources", async () => {
    ...
    try {
      ...
      const shoutoutData: ShoutoutData = {
        message:
          TestDataFactory.generateTestName("COMMUNITY_SHOUTOUT") +
      ...
      await waitFor(
        () => {
          ...
          const found = resourceShoutouts?.some(
            (shoutout) =>
              shoutout.id === communityShoutout.id &&
              shoutout.resourceId === communityResource.id,
          );
          expect(found).toBe(true);
        },
        { timeout: 10000 },
  ...
  test("should handle member participation in community events", async () => {
    if (!communityMember || !communityEvent) {
      console.warn(
        "Skipping community event participation test - setup failed",
      );
      return;
    }
  ...
  test.skip("should create conversations within community context", async () => {
    ...
    const followUpData = {
      content:
        "Thanks for creating this awesome community! Looking forward to participating.",
    ...
    try {
      ...
      await testUtils.performAsyncAction(
        () =>
          shoutoutResult.current.mutateAsync({
            message:
              TestDataFactory.generateTestName("WORKFLOW_SHOUTOUT") +
              " - This resource is exactly what I needed!",
            fromUserId: communityMember.userId,
      ...
      await waitFor(
        () => {
          ...
          const resourceFound = resources?.some(
            (r) => r.id === workflowResource.id,
          );
          ...
          const shoutoutFound = shoutouts?.some(
            (s) => s.resourceId === workflowResource.id,
          );
  ...
  test("should handle cross-feature filtering and queries", async () => {
    ...
    await waitFor(
      () => {
        ...
        expect(Array.isArray(result.current.communityResources.data)).toBe(
          true,
        );
