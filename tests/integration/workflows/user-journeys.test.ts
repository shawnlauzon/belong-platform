...
describe('Multi-Step Workflow Integration Tests', () => {
  ...
  test.skip('content creation and interaction cycle', async () => {
    ...
    expect(
      eventAttendees.some((attendee) => attendee.user.id === userBAccount.id)
    ).toBe(true);
    ...
    expect(shoutoutsReceived.some((s) => s.resourceId === resource.id)).toBe(
      true
    );
  ...
  test('multi-user collaboration workflow', async () => {
    ...
    for (let i = 0; i < userAccounts.length; i++) {
      ...
      const eventsToJoin = createdContent.filter(
        (c) =>
          c.type === 'event' &&
      ...
      const resourcesToShoutout = createdContent.filter(
        (c) =>
          c.type === 'resource' &&
        ...
        await testUtils.performAsyncAction(
          () =>
            shoutoutsResult.current.create({
              ...
              toUserId:
                resourceToShoutout.item.owner?.id || userAccounts[0].account.id,
            }),
    ...
    for (const eventContent of createdContent.filter(
      (c) => c.type === 'event'
    )) {
