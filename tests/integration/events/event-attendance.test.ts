...
describe('Event Attendance Integration', () => {
  let testUser: any;
  let secondUser: any;
  ...
  test('should handle event capacity limits', async () => {
    ...
    if (secondUser) {
      try {
        ...
        const { result: secondJoinResult } = await testUtils.renderHookWithWrapper(() => 
          useJoinEvent()
        );
  ...
  test('should handle attendance for past events', async () => {
    ...
    try {
      ...
      console.log('✅ Past event join allowed (system permits retrospective attendance)');
    } catch (error) {
