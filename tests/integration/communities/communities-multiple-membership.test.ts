...
describe('Communities API - Multiple Membership Operations', () => {
  ...
  beforeAll(async () => {
    ...
    // Sign in as organizer to create communities
    await signIn(supabase, organizerEmail, 'TestPass123!');
    ...
    // Sign out organizer, sign in as test user for membership tests
    await signOut(supabase);
    await signIn(supabase, testUserEmail, 'TestPass123!');
  });
  ...
  describe('Multiple Community Membership', () => {
    afterAll(async () => {
      // Clean up all memberships
      await cleanupMembership(community1.id, testUser.id);
      await cleanupMembership(community2.id, testUser.id);
      await cleanupMembership(community3.id, testUser.id);
      await signOut(supabase);
    });
    ...
    it('allows user to join multiple communities', async () => {
      ...
      const userCommunities = await api.fetchUserCommunities(supabase, testUser.id);
    ...
    it('prevents duplicate membership across multiple joins', async () => {
      ...
      await expect(
        api.joinCommunity(supabase, community1.id)
