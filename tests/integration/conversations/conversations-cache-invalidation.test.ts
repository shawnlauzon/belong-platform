  ...
  describe('Message Cache Invalidation', () => {
    test('should invalidate conversations cache when sending new message', async () => {
      ...
      await waitFor(() => {
        ...
        expect(updatedConversations[0].lastMessage?.content).toBe(
          'Cache invalidation test message'
        );
      });
  ...
  describe('Cross-User Cache Consistency', () => {
    test('should maintain cache consistency across multiple conversation queries', async () => {
      ...
      await waitFor(() => {
        ...
        const updatedConversations =
          conversationsResult.current.data?.pages[0]?.data || [];
        const conversation1 = updatedConversations.find(
          (c) =>
            c.participants.includes(user1.id) &&
            c.participants.includes(user2.id)
        );
        expect(conversation1?.lastMessage?.content).toBe(
          'Updated message to User 2'
        );

        // Messages for conversation 1 should be updated
        const updatedMessages1 =
          messages1Result.current.data?.pages[0]?.data || [];
