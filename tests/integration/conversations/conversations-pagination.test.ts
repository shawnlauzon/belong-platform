...
describe("Conversations Pagination Integration Tests", () => {
  ...
  describe("Message Pagination", () => {
    test("should paginate messages in a conversation with many messages", async () => {
      ...
      const firstPageMessages = messagesResult.current.data?.pages[0]?.data || [];
      ...
      if (messagesResult.current.hasNextPage) {
        ...
        const secondPageMessages = messagesResult.current.data?.pages[1]?.data || [];
  ...
  describe("Conversation List Pagination", () => {
    test("should paginate conversation list with many conversations", async () => {
      ...
      const firstPageConversations = conversationsResult.current.data?.pages[0]?.data || [];
      ...
      if (conversationsResult.current.hasNextPage) {
        ...
        await waitFor(() => {
          expect(conversationsResult.current.data?.pages?.length).toBeGreaterThan(1);
        });
      }

      // Count total conversations across all pages
      const allConversations = conversationsResult.current.data?.pages?.flatMap(page => page.data) || [];
      ...
      for (let i = 0; i < allConversations.length - 1; i++) {
        const currentTime = new Date(allConversations[i].lastMessage?.createdAt || 0).getTime();
        const nextTime = new Date(allConversations[i + 1].lastMessage?.createdAt || 0).getTime();
        expect(currentTime).toBeGreaterThanOrEqual(nextTime);
      }
    ...
    test("should handle conversation list pagination with mixed activity", async () => {
      ...
      let conversation1Id: string, conversation2Id: string, conversation3Id: string;
      ...
      expect(conversations[0].lastMessage?.content).toBe("Updated first conversation");
  ...
  describe("Pagination Edge Cases", () => {
    ...
    test("should maintain pagination state during real-time updates", async () => {
      ...
      const initialMessageCount = messagesResult.current.data?.pages[0]?.data?.length || 0;
      ...
      await waitFor(() => {
        ...
        expect(updatedMessages[0].content).toBe("New message during pagination");
      });
