...
describe('Conversations Integration Tests', () => {
  ...
  test('should support message content variations', async () => {
    ...
    const longMessage = TestDataFactory.createMessage({
      content:
        'This is a very long message that contains multiple sentences and should be handled properly by the conversation system. It includes various punctuation marks and special characters! @ # $ % ^ & * ( ) - _ = + [ ] { } | \ : ; " \' < > , . ? / ~ `',
    });
  ...
  test('should validate message threading concepts', async () => {
    ...
    expect(messages[1].createdAt.getTime()).toBeGreaterThan(
      messages[0].createdAt.getTime()
    );
    expect(messages[2].createdAt.getTime()).toBeGreaterThan(
