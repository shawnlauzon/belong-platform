...
describe('Error Handling and Resilience Integration', () => {
  ...
  test('should handle network interruption simulation', async () => {
    ...
    try {
      ...
      console.warn('Resource creation failed even under normal conditions:', error);
  ...
  test('should handle resource not found errors', async () => {
    ...
    const { result: invalidCommunityResult } = await testUtils.renderHookWithWrapper(() => 
    ...
    const { result: invalidOrganizerResult } = await testUtils.renderHookWithWrapper(() => 
  ...
  test('should handle concurrent operation conflicts', async () => {
    ...
    const concurrentPromises = [
      result.current({ ...baseResourceData, title: baseResourceData.title + '_1' }),
      result.current({ ...baseResourceData, title: baseResourceData.title + '_2' }),
      result.current({ ...baseResourceData, title: baseResourceData.title + '_3' }),
      result.current({ ...baseResourceData, title: baseResourceData.title + '_4' }),
      result.current({ ...baseResourceData, title: baseResourceData.title + '_5' }),
    ];
  ...
  test('should handle extremely large data payloads', async () => {
    ...
    const largeResourceData: ResourceData = {
      ...
      category: 'tools',
      communityId: testCommunity.id,
      imageUrls: Array.from({ length: 100 }, (_, i) => `https://example.com/image${i}.jpg`), // Many URLs
      isActive: true,
    };
