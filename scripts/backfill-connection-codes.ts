...
async function findMembersWithoutCodes() {
  ...
  return memberships?.filter(m => 
    !existingCodesSet.has(`${m.user_id}-${m.community_id}`)
  ).map(m => ({
    user_id: m.user_id,
...
async function backfillConnectionCodes(dryRun: boolean = false) {
  try {
    ...
    console.log(`📋 Found ${membersWithoutCodes.length} members without connection codes:\n`);
    
    // Group by community for better readability
    const membersByCommunity = membersWithoutCodes.reduce((acc, member) => {
    ...
    if (dryRun) {
      ...
      console.log(`Would generate ${membersWithoutCodes.length} connection codes`);
      return;
    }
    ...
    for (const member of membersWithoutCodes) {
      try {
        console.log(`Generating code for ${member.user_email} in ${member.community_name}...`);
    ...
    if (failures.length > 0) {
      ...
      failures.forEach(({ member, error }) => {
        console.log(`  - ${member.user_email} in ${member.community_name}: ${error}`);
      });
