...
export async function seedResources() {
  ...
  try {
    ...
    if (count && count > 0) {
      logger.info('🌱 Resources already seeded, skipping...', { existingCount: count });
      return;
    }
    ...
    logger.info('🌱 Seeding resources for user:', { userId: user.id, resourceCount: mockResources.length });
    logApiCall('POST', '/resources/seed', { userId: user.id, count: mockResources.length });
    ...
    logApiResponse('POST', '/resources/seed', { success: true, count: mockResources.length });
    logger.info('✅ Successfully seeded resources', { count: mockResources.length });
  } catch (error) {
