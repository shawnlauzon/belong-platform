...
export async function fetchResources(
  ...
  const needsCommunityFilter = filters?.communityId || (filters?.communityIds && filters.communityIds.length > 0);
  
  if (needsCommunityFilter) {
    // Build community filter for junction table
    const communityIds = filters?.communityIds && filters.communityIds.length > 0 
    ...
    // Now query resources with these IDs
    let query = supabase
      .from('resources')
