...
): CommentMetadata | ShoutoutMetadata | TrustPointsMetadata | TrustLevelMetadata | ResourceUpdatedMetadata | Record<string, never> {
  switch (type) {
    case 'comment':
    case 'comment_reply':
      return {
        content_preview: typeof metadata.content_preview === 'string' ? metadata.content_preview : ''
      };
    ...
    case 'trust_level_changed':
      return {
        old_level: typeof metadata.old_level === 'number' ? metadata.old_level : 0,
        new_level: typeof metadata.new_level === 'number' ? metadata.new_level : 0
      };

    case 'claimed_resource_updated':
      return {
        changes: Array.isArray(metadata.changes) && metadata.changes.every(c => typeof c === 'string') 
