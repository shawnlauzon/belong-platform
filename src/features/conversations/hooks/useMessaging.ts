...
export function useMessaging(
  ...
  const markAsReadMutation = useMutation({
    ...
    onError: (error) => {
      logger.error('💬 useMessaging: Failed to mark message as read', {
        error,
      });
    },
  });
  ...
  const result = {
    ...
    sendMessage: (data: MessageData) => {
      return sendMessageMutation?.mutateAsync
        ...
        : Promise.reject(new Error('Send message mutation not ready'));
    },
    markAsRead: (messageId: string) => {
      return markAsReadMutation?.mutateAsync
        ? markAsReadMutation.mutateAsync(messageId)
        : Promise.reject(new Error('Mark as read mutation not ready'));
    },
