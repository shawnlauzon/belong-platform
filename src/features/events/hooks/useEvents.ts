...
import { STANDARD_CACHE_TIME, SHORT_CACHE_TIME } from '../../../config';
import type {
...
export function useEvents() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async (_, eventId) => {
      ...
      const eventsQueries = allQueries.filter(
        (q) => q.queryKey[0] === 'events' || q.queryKey[0] === 'event'
      );
  ...
  return {
    ...
    leave: (eventId: string) => {
      return leaveMutation?.mutateAsync
        ? leaveMutation.mutateAsync(eventId)
