...
import type { CommunityInfo, CommunityData } from '../types/domain';
...
export function useCommunities() {
  ...
  if (!queryClient) {
    throw new Error(
      'QueryClient not available. Make sure your component is wrapped with QueryClientProvider.'
    );
  }
  ...
  const result = {
    ...
    join: (communityId: string, role?: 'member' | 'admin' | 'organizer') =>
      joinMutation?.mutateAsync
        ...
        : Promise.reject(new Error('Join mutation not ready')),
    leave: (communityId: string) => {
      return leaveMutation?.mutateAsync
        ? leaveMutation.mutateAsync(communityId)
        : Promise.reject(new Error('Leave mutation not ready'));
    },
