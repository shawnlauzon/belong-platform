...
export function useMessages(conversationId: string): UseMessagesResult {
  ...
  useEffect(() => {
    if (!client || !conversationId || isConversationLoading || !conversation || isCurrentUserLoading || !currentUser) {
    ...
    const setupRealtimeMessaging = async () => {
      try {
          ...
          .on(
            ...
            async (payload) => {
              ...
              if (data) {
                ...
                logger.warn('useMessages: failed to fetch updated message data', {
                  messageId: payload.new.id,
                  conversationId
                });
              }
        ...
        logger.info('useMessages: realtime subscription established successfully', {
          conversationId,
