...
export class ThanksManager {
  static async getThanksFeed(communityId?: string): Promise<Thanks[]> {
    ...
    try {
      ...
      const transformedThanks: Thanks[] = thanks.map(thank => {
        ...
        return {
          ...
          from_member: thank.from_user ? {
            ...
            name: fromUserMetadata.full_name || thank.from_user.email?.split('@')[0] || 'Anonymous',
            ...
            thanks_received: 0,
            resources_shared: 0,
            created_at: thank.from_user.created_at || new Date().toISOString()
          } : undefined,
          to_member_id: thank.to_user_id,
          to_member: thank.to_user ? {
            id: thank.to_user.id,
            name: toUserMetadata.full_name || thank.to_user.email?.split('@')[0] || 'Anonymous',
            ...
            thanks_received: 0,
            resources_shared: 0,
            created_at: thank.to_user.created_at || new Date().toISOString()
          } : undefined,
          ...
          resource: thank.resource ? {
            ...
            owner: thank.resource.creator ? {
              ...
              trust_score: 5.0,
              location: resourceOwnerMetadata.location || { lat: 0, lng: 0 },
              community_tenure_months: 0,
              thanks_received: 0,
              resources_shared: 0,
      ...
      logger.info('🙏 ThanksManager: Thanks feed retrieved:', { count: transformedThanks.length });
    ...
    try {
      ...
      const newThanks: Thanks = {
        ...
        from_member: createdThanks.from_user ? {
          ...
          name: fromUserMetadata.full_name || createdThanks.from_user.email?.split('@')[0] || 'Anonymous',
          first_name: fromUserMetadata.first_name || '',
          last_name: fromUserMetadata.last_name || '',
          avatar_url: fromUserMetadata.avatar_url || null,
          trust_score: 5.0,
          ...
          thanks_received: 0,
          resources_shared: 0,
          created_at: createdThanks.from_user.created_at || new Date().toISOString()
        } : undefined,
        to_member_id: createdThanks.to_user_id,
        to_member: createdThanks.to_user ? {
          id: createdThanks.to_user.id,
          name: toUserMetadata.full_name || createdThanks.to_user.email?.split('@')[0] || 'Anonymous',
          first_name: toUserMetadata.first_name || '',
          last_name: toUserMetadata.last_name || '',
          avatar_url: toUserMetadata.avatar_url || null,
          trust_score: 5.0,
          ...
          thanks_received: 0,
          resources_shared: 0,
          created_at: createdThanks.to_user.created_at || new Date().toISOString()
        } : undefined,
      ...
      if (newThanks.to_member) {
        logger.debug('🙏 ThanksManager: Updating trust score for recipient:', { memberId: newThanks.to_member.id });
        
        // Recalculate trust score
        const newScore = await TrustCalculator.calculateScore(newThanks.to_member.id);
  ...
  static async getThanksByMemberId(memberId: string): Promise<Thanks[]> {
    ...
    try {
      ...
      logApiResponse('GET', `/thanks/member/${memberId}`, { count: transformedThanks.length });
