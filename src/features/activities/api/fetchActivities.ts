...
import type { ActivityInfo, ActivityFilter, ActivitySection } from '../types';
import { 
...
export async function fetchActivities(
  ...
  try {
    ...
    const [eventsResult, resourcesResult, shoutoutsResult, messagesResult] = await Promise.all([
    ...
    const eventActivities = transformEventsToActivities(eventsResult.data || []);
    const resourceActivities = transformResourcesToActivities(resourcesResult.data || []);
    const shoutoutActivities = transformShoutoutsToActivities(shoutoutsResult.data || []);
    const messageActivities = transformMessagesToActivities(messagesResult.data || []);
...
async function fetchPendingShoutouts(
  ...
  const shoutoutResourceIds = new Set(existingShoutouts?.map(s => s.resource_id) || []);
  
  // Filter out resources that already have shoutouts
  const pendingShoutouts = data.filter(r => !shoutoutResourceIds.has(r.resource_id));
 ...
 */
function filterBySection(activities: ActivityInfo[], section: ActivitySection): ActivityInfo[] {
  const now = new Date();
