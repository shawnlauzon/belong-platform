...
import { fetchResourceInfoById } from '../../resources/api/fetchResourceInfoById';
...
export const createShoutoutsService = (supabase: SupabaseClient<Database>) => ({
  ...
  async fetchShoutoutById(id: string): Promise<Shoutout | null> {
    ...
    try {
      const query = supabase.from('shoutouts').select('*').eq('id', id);
      ...
      const [fromUser, toUser, resourceInfo] = await Promise.all([
        fetchUserById(supabase, data.from_user_id),
        fetchUserById(supabase, data.to_user_id),
        fetchResourceInfoById(supabase, data.resource_id),
      ]);
      ...
      const resourceOwner = await fetchUserById(supabase, resourceInfo.ownerId);
      if (!resourceOwner) {
  ...
  async createShoutout(data: ShoutoutData): Promise<Shoutout> {
    ...
    try {
      ...
      const [fromUser, toUser, resourceInfo] = await Promise.all([
        fetchUserById(supabase, createdShoutout.from_user_id),
        fetchUserById(supabase, createdShoutout.to_user_id),
        fetchResourceInfoById(supabase, createdShoutout.resource_id),
      ]);
  ...
  async updateShoutout(
    ...
    try {
      ...
      // Fetch fromUser, toUser, and resource from cache
      const [fromUser, toUser, resourceInfo] = await Promise.all([
        fetchUserById(supabase, updatedShoutout.from_user_id),
        fetchUserById(supabase, updatedShoutout.to_user_id),
        fetchResourceInfoById(supabase, updatedShoutout.resource_id),
      ]);
