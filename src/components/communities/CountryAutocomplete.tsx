...
export function CountryAutocomplete({ 
  ...
  const filterSuggestions = (query: string) => {
    ...
    const allCountries = [...new Set([...existingCountries, ...COMMON_COUNTRIES])];
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const exactMatch = filtered.find(country => 
      country.toLowerCase() === newValue.toLowerCase()
    );
        ...
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg p-3">
          <div className="flex items-center gap-2 text-gray-500">
            ...
            <span className="text-sm">
              No countries found. Press Enter to add "{inputValue}" as a new country.
            </span>
          </div>
        </div>
      )}
