...
export function ResourceMap({ resources, userLocation, onRequestResource }: ResourceMapProps) {
        ...
        {resources.map(resource => (
          ...
          >
            <div className="relative cursor-pointer transform transition-transform hover:scale-110">
              <div className={`h-8 w-8 ${resource.type === 'offer' ? getCategoryColor(resource.category) : 'bg-primary-500'} rounded-full flex items-center justify-center text-white`}>
                <MapPin className="h-4 w-4" />
              </div>
              <div className="absolute -bottom-1 -right-1">
                <TrustBadge score={resource.owner?.trust_score || 0} size="xs" />
              </div>
            </div>
          </Marker>
        ))}
